...
const BookingSuccessEmptyLeg = () => {
    const flight = useSelector((store) => store.flightTicket);
    const flightId = flight._id;
    const generateInvoice = async (flightId) => {
        try {
            const response = await axios.get(
            `${import.meta.env.VITE_API_BASE_URL}/empty/invoice/${flightId}`,
            {
    ...
    return (
        ...
        <button
            onClick={() => {generateInvoice(flightId)}}
            style={{
            backgroundColor: "#061953",
            color: "white",
            padding: "12px 24px",
    ...
    export default BookingSuccessEmptyLeg;